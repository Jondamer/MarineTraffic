...
# data = pd.read_csv('outPut/xiboPOIDbscan2_2', header=None)
# data = pd.read_csv('outPut/singaporePOICuzhongxindian', header=None)
# data = pd.read_csv('inData/maoboPOICombine16bitsGridby20.csv', header=None,sep=',')
data = pd.read_csv('inData/allShipsFullYearby50', header=None, sep=',')
...
map.drawcoastlines(linewidth=0.5, linestyle='solid', color='blue', antialiased=1, ax=None, zorder=None)
map.drawcountries()
...
def mapshow():
	...
	lon = np.array(posi_fea[0][1:])
	lat = np.array(posi_fea[1][1:])
	size = 5
	# ax = plt.gca()
	x, y = map(lon, lat)
	...
	plt.title(' halfyear16bitsGridby0 ')
	plt.show()
