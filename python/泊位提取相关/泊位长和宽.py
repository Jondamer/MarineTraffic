...
while (i < n):
	...
	for j in range(i + 1, n):
		...
		if (a != None and b != None and a.intersects(b)):
			...
			if ((pyo.area / a.area) > 0.3 or (pyo.area / b.area) > 0.3):
				print('相交区域的交集是', pyo, pyo.centroid.x, pyo.centroid.y)
				print('需要合并', i, j, a, b)
...
with open('amstanPolygontest7-3V4', 'w') as f:
	for i in range(lsl):
		if (listpolygon[i] != None):
			# print(listpolygon[i])
			f.write(str(listpolygon[i]) + ";" + "Amsterdamv4" + ";" + str(listpolygon[i].centroid.x) + " " +
